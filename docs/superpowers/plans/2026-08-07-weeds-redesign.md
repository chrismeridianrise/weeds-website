...
- **`accent` `#FFCA57` is the only bright color on the page.** A gold *fill* marks a ticket link or the single primary CTA of a section — nothing else. Several gold buttons may be visible at once when they are the same action (the Shows list gives every show its own Tickets button, which is correct and intended). What is forbidden is gold filling a non-CTA element, or a section offering two *different* gold-filled actions and so splitting the eye. Gold as a *text or icon* color is permitted and intended for small labels: eyebrows, instrument credits, show notes, social icons, and disclosure toggles.
